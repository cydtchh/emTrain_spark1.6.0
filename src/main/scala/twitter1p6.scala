...
object twitter1p6 {
  ...
  val unlabeledPath = "/home/cai/DM/twitter/differentUnlabeled/labeled10"
...
//    val unlabeledPath = args(0)
    ...
    val corpusLabeled = sqlContext.createDataFrame(weightedLabeled, weightStructure) //Schema: features, weight, label
    //      .repartition(200)
    ...
    val corpusTest = model.transform(hashingTF.transform(dfTest))
      .drop(originalLabel)
    //      .repartition(200)
    ...
    print(s"current unlabeled set $unlabeledPath")
    ...
    for (trial <- 1 to NUM_REPLICATIONS) {
      ...
      val numUnlab = trainUnlabeled.count()
      ...
      val model = EMCore.emTrainNaiveBayes(corpusLabeled, trainUnlabeled, numLabel, sc, smoothing, minImprovement
    ...
    println(s"Supervised mean(acc)=$meanAccSup  sd(Sup)=$sdAccSup    EM mean(acc)=$meanAcc  sd(EM)=$sdAcc")
