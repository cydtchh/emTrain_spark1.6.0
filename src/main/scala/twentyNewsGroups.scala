...
object twentyNewsGroups {
  ...
  val smoothing = 0.01D    //0.007D
  val NUM_REPLICATIONS: Int = 10
  val newSchema = Seq("label", "maxInd")
  val minImprovement = 0.00001//0.0013 0.0004
  val maxEpoch = 20
...
//    .add(StructField(label, DoubleType, nullable = false))
  ...
  def main(args: Array[String]){
    ...
    val EMCore2 = new emCore().setMaxEpoch(maxEpoch).setUnlabeledWeight(1.0D)
...
//        Row(x.getAs[Vector](feature), weightVector, x.getAs[Double](label))
    ...
    for(numSupervisedItems <- NumSupervisedItems){
      ...
      for(trial <- 1 to NUM_REPLICATIONS) {
        ...
        val completeWeight = EMCore2.emTrainNaiveBayes(labeledTrain, unlabeledTrain, numLabel, sc, smoothing, minImprovement
          , corpusTest, numofTestItems)
        val comWeightPrediction = completeWeight.transform(corpusTest)
        accSup(trial-1) = EMCore2.AccuracyCal(comWeightPrediction.select(label, "prediction"), sc, numofTestItems)
...
//        accSup(trial-1) = EMCore.AccuracyCal(supervisedPrediction.select(label, "prediction"), sc, numofTestItems)
        ...
        println(s"Complete Weight EM Accuracy: $accSupToShow      EM Accuracy: $accToShow")
        println("-----------------------------------------------------------------------------------------------------")
      ...
      println(s"#Sup=$numSupervisedItems     Complete Weight mean(acc)=$meanAccSup  sd(Sup)=$sdAccSup    EM mean(acc)=$meanAcc  sd(EM)=$sdAcc")
      println("=====================================================================================================")
      println()
    }
